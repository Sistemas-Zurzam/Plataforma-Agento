 ...
 */
export default function AreaSelect({ empresaId, value, onChange, disabled, onCreateError }) {
  ...
  if (busquedaNormalizada && !coincideExistente) {
    options.push({
      value: CREAR_AREA,
  ...
  return (
    <Select
      ...
      disabled={disabled}
      notFoundContent={loading ? 'Buscando…' : 'Escribe para crear un área'}
      options={options}
    />
  );
}
